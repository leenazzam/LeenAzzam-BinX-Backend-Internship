...
/// <summary>
/// Gets appointments accessible to the authenticated user.
/// </summary>
/// <response code="200">Appointments returned successfully.</response>
[HttpGet]
    ...
    public async Task<ActionResult<IEnumerable<AppointmentResponseDto>>> GetAppointments()
    {
        ...
        if (User.IsInRole("Patient") && !User.IsInRole("Admin") && !User.IsInRole("Doctor"))
        {
            ...
            query = query.Where(a => a.PatientId == parsedPatientId);
        }
...
/// <response code="404">Appointment was not found or is not accessible.</response>
[HttpGet("{id}")]
    [Authorize(Roles = "Admin,Doctor,Patient")]
...
[HttpPost]
    ...
    public async Task<ActionResult<AppointmentResponseDto>> AddAppointment(AppointmentRequestDto request)
    {
