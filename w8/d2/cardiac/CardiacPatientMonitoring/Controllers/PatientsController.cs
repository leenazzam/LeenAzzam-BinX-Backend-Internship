...
public class PatientsController : ControllerBase
{
    ...
    // A Patient can view their own profile; Admin/Doctor can view any profile.
    [HttpGet("{id}")]
...
[HttpGet("summary")]
[Authorize(Roles = "Admin,Doctor")]
public async Task<ActionResult<List<PatientSummaryDto>>> GetPatientsSummary()
{
    var summaries = await _context.Patients
        .Select(p => new PatientSummaryDto
        {
            Id = p.Id,
            FullName = p.FullName,
            AlertCount = _context.Alerts.Count(a => a.PatientId == p.Id)
        })
        .ToListAsync();
