...
if (app.Environment.IsDevelopment())
{
    ...
    if (context.Projects.Count() < 20)
    {
        var random = new Random();
        var adminId = "f0e5b283-ea85-4462-b567-d0f95f649ff8";
        ...
        for (int i = 1; i <= 20; i++)
        {
            ...
            {
                Name = $"Seed Project {i}",
                Description = $"Auto-generated project number {i} for load testing",
                CreatedDate = DateTime.UtcNow.AddDays(-random.Next(1, 100)),
                OwnerId = adminId
            };
            ...
            var taskCount = random.Next(3, 6);
            for (int j = 1; j <= taskCount; j++)
            {
                context.Tasks.Add(new AppTask
                {
                    Title = $"Seed Task {i}-{j}",
                    Status = j % 2 == 0 ? "Done" : "Pending",
